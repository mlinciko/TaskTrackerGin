...
run:
	set -a; source $(SOURCE_FOLDER)/.env; set +a; go run $(SOURCE_FOLDER)/main.go
...
docker-run:
	docker run --env-file $(SOURCE_FOLDER)/.env -p 8080:8080 $(APP_NAME)
...
migrate:
	set -a; source $(SOURCE_FOLDER)/.env; set +a; go run cmd/migrate.go
